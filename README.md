...
| Q21 | Which firms are refused repeatedly, and does repetition precede an import alert? | ~6 months | needs the archive to see new alerts land. The back-catalogue is here: **55,875 firm-days across 31,253 firms, 2001–2026** |
...
| Q24 | Does an import alert stop the refusals, or run alongside them? | nothing | **answered, and they diverge.** Germany: 3.4 refusal firm-days per site on 0.8 alerts per 100. China: 0.5 refusals on 14.1 alerts — the inverse. An alert is standing detention without physical examination, so an alerted firm stops shipping. The alert is the policy, the refusal is its exercise, and measuring one as a proxy for the other is wrong |
